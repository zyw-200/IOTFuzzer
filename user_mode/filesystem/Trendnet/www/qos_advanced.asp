...
<script type="text/javascript">
	...
	function onPageLoad()
	{
		...
		{
			content += '<option value="' + upload_group.attr[i] + '">'+ upload_group.name[i] +'(Upload)</option>';
		}
		content += "</select>";
	...
	function loadValue(idx)
	{
		...
		switch (qos_rule_obj.protocolIdx[idx])
		{
			case "1":
				...
				$('#dFromPort').val(qos_rule_obj.remotePortStart[idx] =="0" ? "" : qos_rule_obj.remotePortStart[idx]);
				$('#dToPort').val(qos_rule_obj.remotePortEnd[idx]=="0" ? "" : qos_rule_obj.remotePortEnd[idx]);
				break;
			case "2":
		...
		$('#mac_address').val(qos_rule_obj.mac[idx]);
		$('#dip_address').val(qos_rule_obj.remoteIPStart[idx]=="0.0.0.0" ? "" : qos_rule_obj.remoteIPStart[idx]);
		$('#sip_address').val(qos_rule_obj.localIPStart[idx]=="0.0.0.0" ? "" : qos_rule_obj.localIPStart[idx]);
		$('#pktlenfrom').val(qos_rule_obj.pkLStart[idx]=="0" ? "" : qos_rule_obj.pkLStart[idx]);
		$('#pktlento').val(qos_rule_obj.pkLEnd[idx]=="0" ? "" : qos_rule_obj.pkLEnd[idx]);
		set_selectIndex(qos_rule_obj.dscp[idx], $("#dscp")[0]);
		set_selectIndex(qos_rule_obj.Remarkdscp[idx], $("#remark_dscp")[0]);
	...
	function send_request()
	{
		...
		obj.set_param_next_page('qos_advanced.asp');
		var inst = '1.1.1.'+idx;
		...
		obj.add_param_arg('trafficRule_RemoteIPAddrStart_',inst,$('#dip_address').val());
		obj.add_param_arg('trafficRule_LocalIPAddrStart_',inst,$('#sip_address').val());
		obj.add_param_arg('trafficRule_PacketLenStart_',inst,$('#pktlenfrom').val());
		obj.add_param_arg('trafficRule_PacketLenEnd_',inst,$('#pktlento').val());
		obj.add_param_arg('trafficRule_DSCP_',inst,$('#dscp').val());
		...
		if($('#protocol').val() != "")
		{
			...
			if($('#protocol').val() == "TCP" || $('#protocol').val() == "UDP")
			{
				...
				obj.add_param_arg('trafficRule_RemotePortStart_',inst,$('#dFromPort').val());
				obj.add_param_arg('trafficRule_RemotePortEnd_',inst,$('#dToPort').val());
			}
		...
		{
			obj.add_param_arg('trafficRule_Protocol_',inst,'');
			obj.add_param_arg('trafficRule_LocalPortStart_',inst,'');
			obj.add_param_arg('trafficRule_LocalPortEnd_',inst,'');
			obj.add_param_arg('trafficRule_RemotePortStart_',inst,'');
			obj.add_param_arg('trafficRule_RemotePortEnd_',inst,'');
			obj.add_param_arg('trafficRule_AppName_',inst,'');
		}
		
		if($('#remark_dscp').val() != "")
			obj.add_param_arg('trafficRule_RemarkDSCP_',inst,$('#remark_dscp').val());
	...
	function checkdata()
	{
		...
		var dip = $('#dip_address').val();
		var dip_addr_msg = replace_msg(all_ip_addr_msg,get_words('_ipaddr'));
		var temp_dip_obj = new addr_obj(dip.split("."), dip_addr_msg, false, false);
		
		var sip = $('#sip_address').val();
		var sip_addr_msg = replace_msg(all_ip_addr_msg,get_words('_ipaddr'));
		var temp_sip_obj = new addr_obj(sip.split("."), sip_addr_msg, false, false);
		...
		if(MAC=="" && dip=="" && sip=="" && $('#pktlenfrom').val() =="" && $('#dscp').val() =="-1" && $('#protocol').val() =="")
		{
			alert(get_words('_qos_txt46'));
			return false;
		}
		...
		if(($('#pktlenfrom').val() !="" && $('#pktlento').val() =="") || ($('#pktlenfrom').val() =="" && $('#pktlento').val() !=""))
		{
			alert(get_words('_qos_txt45'));
			return false;
		}
		if($('#pktlenfrom').val() !=""){
			if(isNaN($('#pktlenfrom').val()) || isNaN($('#pktlento').val()))
			{
				alert(get_words('_qos_txt44'));
				return false;
			}
			
			if($('#pktlenfrom').val() > 2048 || $('#pktlento').val() > 2048)
			{
				alert(get_words('_qos_txt43'));
				return false;
			}
			
			if($('#pktlenfrom').val() < 0 || $('#pktlento').val() < 0)
			{
				alert(get_words('_qos_txt42'));
				return false;
			}		
			var pktlento = parseInt($('#pktlento').val(), 10);
			var pktlenfrom = parseInt($('#pktlenfrom').val(), 10);
				
			if(pktlento < pktlenfrom)
			{
				alert(get_words('_qos_txt41'));
				return false;
			}
		}
		if($('#protocol').val() == "TCP" || $('#protocol').val() == "UDP")
		...
		if($('#protocol').val() == "TCP")
		{
			tcp_timeline = add_into_timeline(tcp_timeline, $('#dFromPort').val(), $('#dToPort').val());
		}
		if($('#protocol').val() == "UDP")
		{
			udp_timeline = add_into_timeline(udp_timeline, $('#dFromPort').val(), $('#dToPort').val());
		}
		if (check_timeline(tcp_timeline) == false) {
			...
			if($('#protocol').val() == "Application")
				...
				alert(addstr(get_words('ag_conflict5'), 'TCP', ($('#dToPort').val()==''?$('#dFromPort').val():$('#dFromPort').val()+'-'+$('#dToPort').val())));
			return false;
		}
		if (check_timeline(udp_timeline) == false) {
			if($('#protocol').val() == "Application")
				...
				alert(addstr(get_words('ag_conflict5'), 'UDP', ($('#dToPort').val()==''?$('#dFromPort').val():$('#dFromPort').val()+'-'+$('#dToPort').val())));
			return false;
		}
...
<table align="center" width="540" border="0" cellpadding="0" cellspacing="0">	
	...
	<tr>
		<td align="right"><script>show_words('_qos_txt22')</script> : <a id="qos_link" href=""><font color="blue">[<script>show_words('_basic')</script>]</font></a> / [<script>show_words('_qos_txt23')</script>]</td>
	</tr>
	</table>
	<tr>		
		...
		<td valign="top">
            ...
            <input type="hidden" id="html_response_return_page" name="html_response_return_page" value="qos_advanced.asp" />
			...
			<tr><td colspan="2" class="CT"><script>show_words('_qos_txt39')</script></td></tr>
			<tr>
			...
			<tr>
				<td align="right" class="CL"><script>show_words('_protocol')</script></td>
				<td class="CR">
			...
			<tr id="remarkDSCP">
				...
				<td class="CR">
					...
					<option value="-1" id="QoSClassifierAutoStr"><script>show_words('KR50')</script></option>
