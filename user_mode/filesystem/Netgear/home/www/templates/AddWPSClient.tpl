								...
								<table class="tableStyle">
									<tr>
										<td class="DatablockContent" style="text-align: center;">	
											<input type="submit" name="NEXT" id="next" value="Next" onclick="setWPSWizardPage(2);return false;" {if $data.wpsSettings.wpsDisable eq '1'}disabled="disabled"{/if}>&nbsp;&nbsp;
										</td>
									</tr>
								</table>
					</td>
				...
				<tr>
					...
					<td class="spacer100Percent paddingsubSectionBody style="text-align:left">
						...
						<p class="font10Bold" align="left">Select a Setup Method</p><br>
							<div align = "left" class = "font10Bold"><input type="radio" id="wpspushbutton" name="wpsButton" onclick="$('pushbutton').style.display='';$('pinbutton').style.display='none';$('pusBtnImg').focus();" {if $data.monitor.wpsSoftwarePushButtonSupport eq '1'}checked{/if}>Push Button(Recommended)</div>
							...
							<div class = "font10Bold" align = "left"><input type="radio" id="wpspinbutton" name="wpsButton" onclick="$('pinbutton').style.display='';$('pushbutton').style.display='none';$('wpsPin').focus()" {if $data.monitor.wpsPinSupport eq '1' and $data.monitor.wpsSoftwarePushButtonSupport eq '0'}checked{/if}>PIN(Personal Identification Number)</div>
							<div id="pinbutton">
    ...
    <input type="hidden" name="pushButtonStatus" id="pushButtonStatus" value={$data.monitor.wpsSoftwarePushButtonSupport}>
	<input type="hidden" name="pinButtonStatus" id="pinButtonStatus" value={$data.monitor.wpsPinSupport}>
<script language="javascript">
	<!--

	toggleWPSButtons()
	createWPSProgressBar();
    var wizardPage = {$smarty.post.wizardPageID|default:"''"};
    var wpsMethod = {$smarty.post.whichWPSMethod|default:"''"};
    var wpsfailmsg = {$smarty.post.whichwpsfailmsg|default:"''"};
    var userPinVal = '{$smarty.post.userPinValue|default:""}';
