...
int read_state(CPUArchState *env); 
int write_state(CPUArchState *env); 
int write_addr(target_ulong addr);
...
/*
	...
	if(gva_arr[i] != 0)
	{
		if(hva_arr[i] < hva_start){
			printf("zyw mapping memory overflow\n");
			exit(32);
		}
		int un_result = target_munmap(gva_arr[i], 1024*4);
		printf("mmap:%x,%x\n",gva_arr[i], hva_arr[i] - hva_start);
		mmappedData = target_mmap(gva_arr[i], 1024*4, PROT_READ|PROT_WRITE, MAP_SHARED, fd, hva_arr[i] - hva_start);
...
*/
    ...
    //target_munmap(0x401000, 0x139a0);
/*
    for(int i=0; i<0xfffff; i++)
    {
    	int mmap_r = target_munmap(i<<12, 1024*4);
    }
*/
/*
...
/*	
    int res = target_mmap(init_regs[32] & 0xfffff000, 1024*4, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0x1302000);
    env->active_tc.PC = init_regs[32];
...
static void handler(int sig_num, siginfo_t *si, void *ptr)
{
   ...
   printf("signal raised at address:%x, %x ", si->si_addr, h2g(si->si_addr));
   printf("signal ip:%x\n", uc->uc_mcontext.gregs[REG_RIP]);
   //uc->uc_mcontext.gregs[REG_RIP] += 6;
   write_addr(h2g(si->si_addr));
   target_ulong mapp_addr;
   read_addr(&mapp_addr);
   printf("map addr:%x:%x\n", h2g(si->si_addr) & 0xfffff000, (mapp_addr - hva_start) & 0xfffff000);
   int res = target_mmap(h2g(si->si_addr) & 0xfffff000, 1024*4, PROT_READ|PROT_WRITE, MAP_SHARED, fd, (mapp_addr - hva_start) & 0xfffff000);
...
{  
    ...
    pipe_fd = open(fifo_name_user, open_mode_full);    
    if(pipe_fd != -1)  
    {  
	res = write(pipe_fd, &cpustate, sizeof(CPUSHSTATE));  
	if(res == -1)  
	{  
		fprintf(stderr, "Write error on pipe\n");  
		exit(EXIT_FAILURE);  
	}  
	printf("write state ok %x,%x\n", sizeof(CPUSHSTATE), res);
        close(pipe_fd);   
    }  
    else  
        exit(EXIT_FAILURE);  
...
{  
    ...
    if(pipe_fd != -1)  
    {  
	res = write(pipe_fd, &addr, sizeof(target_ulong));  
	if(res == -1){  
		fprintf(stderr, "Write addr error:%x\n", addr);  
		exit(EXIT_FAILURE);  
	}  
	printf("write addr ok\n");
        close(pipe_fd);   
    }  
    else { 
	printf("write addr pipe open error\n");
        exit(EXIT_FAILURE);  
    }
    return 1;  
}  
...
{  
    ...
    {  
	printf("before read state\n");
	res = read(pipe_fd, &cpustate, sizeof(cpustate));  
	if(res == -1)  
	{  
		fprintf(stderr, "Write error on pipe\n");  
		exit(EXIT_FAILURE);  
...
//zyw
int read_addr(target_ulong *addr)  
{  
    const char *fifo_name_full = "/pipe/full_cpu_state";  
    int pipe_fd = -1;  
    int res = 0;  
    ...
    if(pipe_fd != -1)  
    {   
	res = read(pipe_fd, addr, sizeof(target_ulong));  
	if(res == -1)  
	{  
...
int add_mapping(int gva, int hva)
{
	int mmap_ur = target_mmap(gva, 1024*4, PROT_READ|PROT_WRITE, MAP_SHARED, fd, hva - hva_start);
}
...
int remapping(int gva, int hva)
{
	int gva_h = gva & 0xfffff000;
	int hva_h = hva & 0xfffff000;
	if(search_map_table(gva_h)==-1){
		store_mapping(gva_h, hva_h);
		add_mapping(gva_h, hva_h);
	}
}
