...
extern void afl_request_map(target_ulong vaddr, target_ulong paddr);
char * aflFile;
//zyw
...
int gva_arr[MAP_SIZE];
int hva_arr[MAP_SIZE];
int gva_start[VIRTUAL_SPA];
uintptr_t hva_st[VIRTUAL_SPA];
int gva_end[VIRTUAL_SPA];
int gva_type[VIRTUAL_SPA];
int gva_idx[VIRTUAL_SPA];
int vir_index;
int map_index;
int init_regs[36];
uintptr_t hva_start;
...
unsigned long reserved_va = 0x80000000; //user_space
# else
...
void handle_store(target_ulong page_addr)
{
    ...
    if(!store_res)
    {
        ...
        //{
            ...
            page.mmu_idx = 2;
            write_addr(&page);
            read_addr(&page, &mmap_addr);
            int file_offset = (mmap_addr - hva_start) & 0xfffffffffffff000;
            target_mmap(page_addr, 1024*4, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED,  mem_file, file_offset);
...
/*
            ...
            {
                char file_name[100];
                printf("stat:%x, %x, ", env->active_tc.gpr[4], g2h(env->active_tc.gpr[4]));
    		...
    		{
    			char file_name[100];
    			printf("open:%x, %x, ", env->active_tc.gpr[4], g2h(env->active_tc.gpr[4]));
    			cpu_memory_rw_debug(NULL, env->active_tc.gpr[4], file_name,  100, 0);
    			printf("%s\n", file_name);
    		}
            if(syscall_num == 213)
            {
                char file_name[100];
                printf("stat64:%x, %x, ", env->active_tc.gpr[4], g2h(env->active_tc.gpr[4]));
                cpu_memory_rw_debug(NULL, env->active_tc.gpr[4], file_name,  100, 0);
                printf("%s\n", file_name);
            }
*/
           ...
           //else if(syscall_num == 142) // doprbear / tplink httpd _newselect, if we choose this, the memory state is different when restoring.
            ...
            //else if(syscall_num == 45) //Netgear lighttpd 
            //else if(syscall_num == 2) //tmp Trendnet jjhttpd 
            //else if(syscall_num == 116) //tmp tplink httpd syscall 0
            //else if(syscall_num == 171 || syscall_num == 142) //tmp tplink httpd 
    		{
...
//sned 178
            ...
            if(syscall_num == 5)
            {
                ...
                printf("open:%x, %x, ", env->active_tc.gpr[4], g2h(env->active_tc.gpr[4]));
                cpu_memory_rw_debug(NULL, env->active_tc.gpr[4], file_name,  100, 0);
                if(strstr(file_name, "console")==NULL)
                    user_mode = 1;
            }
            */
            //if(0)
            ...
            //if(syscall_num != 45) //cgibin
            //if(syscall_num == 146 || syscall_num == 20) //sample
            {

                /*
                //some addition e.g. read/write/close fd = pipe_read_fd/pipe_write_fd, push to full system mode
                if(env->active_tc.gpr[4] == pipe_write_fd || env->active_tc.gpr[4] == pipe_read_fd)
                {
                    env->active_tc.PC -= 4; //very important
                    write_state(cs->env_ptr);
                    //print_cpu_state(cs->env_ptr);
                    read_state(env->active_tc.PC, cs->env_ptr);
                    ret = env->active_tc.gpr[2];
                }
                */
                //printf("syscall locally:%d\n", syscall_num);
                ...
                printf("syscall end:%x,%x\n", ret, env->active_tc.gpr[7]);
            ...
            {
                //need to push state to full system end
                env->active_tc.PC -= 4; //very important
...
CPUArchState *cpu_copy(CPUArchState *env)
{
    ...
    QTAILQ_FOREACH(wp, &cpu->watchpoints, entry) {
        cpu_watchpoint_insert(new_cpu, wp->_vaddr, wp->len, wp->flags, NULL);
    }
...
int parse_mapping_table(char *filename)
{
    ...
    while(fgets(strline, 100, fp)!=NULL){
        ...
        char *p2 = strtok(NULL, ":");
        char *p3 = strtok(NULL, ":");
        gva_start[vir_index] = strtol(p1,NULL, 16);
        gva_end[vir_index] = strtol(p2,NULL, 16);
        gva_type[vir_index] = strtol(p3,NULL, 16);
        vir_index ++;
    }
#else
	while(fgets(strline, 100, fp)!=NULL){
        ...
        char *p3 = strtok(NULL, ":");
		gva_start[vir_index] = strtol(p1,NULL, 16);
        hva_st[vir_index] = strtol(p2,NULL, 16);
        gva_type[vir_index] = strtol(p3,NULL, 10);
		vir_index ++;
	}
#endif
	fclose(fp);
...
extern target_ulong feed_input_vaddr;

int main(int argc, char **argv, char **envp)
{


//memory for mmaped virtual address
    ...
    printf("hva_start:%lx\n", hva_start);
    mem_file = open("mem_file", O_RDWR, 0);
    assert(mem_file!=-1);
...
# endif
    ...
    if (reserved_va || have_guest_base) {
        guest_base = init_guest_space(guest_base, reserved_va, 0,
                                      have_guest_base);
        //zyw
        printf("guest base:%lx,%lx\n", guest_base, reserved_va);
...
//zyw
    //printf("start:%lx,%lx,%lx,%lx,%lx,%lx,%lx\n", info->start_brk, info->start_code, info->end_code, info->start_data, info->end_data, info->start_stack, info->entry);
...
/*
    for(int i= 0; i< vir_index; i++)
    {
    	if(gva_start[i]!=0)
    	{
            printf("%x:%x\n", gva_start[i], gva_end[i]); 
    	    for(int j = gva_start[i] >> 12; j < gva_end[i] >> 12; j++)
    	    {
        		int res = target_munmap(j<<12, 1024*4);
        		if(res!=0) {printf("munmap failed:%x\n", j<<12);}	
    	    }
    	}
    	else{
    	    break;
...
//lighttpd
        ...
        */
        ...
        for(int i = gva_start[k]; i < gva_end[k]; i = i+ 1024*4)
        {
            ...
            if(page.prot == 0)
            {
                target_mmap(i, 1024*4, PROT_READ, MAP_SHARED | MAP_FIXED,  mem_file, (mmap_addr- hva_start) & 0xfffffffffffff000); 
            }
            else
            {
                target_mmap(i, 1024*4, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED,  mem_file, (mmap_addr- hva_start) & 0xfffffffffffff000);
                store_page(i, 0);
        ...
        if(gva_type[k] == 0 || gva_type[k] == 2)

        {
            res = target_mmap(gva_start[k] & 0xfffff000, 1024*4, PROT_READ, MAP_SHARED | MAP_FIXED,  mem_file, (hva_st[k] - hva_start) & 0xfffffffffffff000);  
        }
        else
        {
            res = target_mmap(gva_start[k] & 0xfffff000, 1024*4, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED,  mem_file, (hva_st[k] - hva_start) & 0xfffffffffffff000);
            store_page(gva_start[k] & 0xfffff000, 0);
            addr_pair[addr_pair_index].virt_page = gva_start[k] & 0xfffff000;
            addr_pair[addr_pair_index].phys_page = (hva_st[k] - hva_start) & 0xfffffffffffff000;
            addr_pair_index++;
            add_physical_page((hva_st[k] - hva_start) & 0xfffffffffffff000);
            add_store_write_page(gva_start[k] & 0xfffff000);
        }
        printf("physical page:%lx, %x\n", (hva_st[k] - hva_start) & 0xfffffffffffff000,gva_type[k]);


/* feed input
        if(((mmap_addr - hva_start) & 0xfffff000) == 0xeda1000)//0x4194b0
        {//grep  grep -r '4800 5454 5f50 4f43' HTTP_COOKIE
           // grep 'fff5 1440 1821 0072 8324 8f82' cgibin
           // grep 'a36c 2484 f809 0320'cgibin 0x40959c
            feed_input_vaddr = (gva_start[k] & 0xfffff000) + 0xcf7;
        }
*/
#endif
    }
    ...
    lib_start_pc = init_regs[32] & 0xfffff000;
//zyw
...
int search_http_and_replace(char *buf , int total_buf_len, int tmp_len, char *search_str, char *replace_str, int buf_len)
{
//split
    char *delim = ' ';
    char *p;
    ...
    index --;
    printf("index is %d\n", index);
    if(strlen(replace_str) + tmp_len > buf_len)
    {
        printf("buf max len is to little\n");
        sleep(1000);
    }
...
//search
    int new_str_index = 0;
    char new_str[3000]; //need to be buf_len - tmp_len
    for(int k= 0; k <= index; k++)
    {
        char * res = strstr(sub_str[k], search_str);
        if(res)
        {
...
//merge
    ...
    int h = 0;
    int total_len = 0;
    char * new_tmp = new_buf;
    while(h<=index){
        if(h != new_str_index)
        {
            strcpy(new_tmp, sub_str[h]);
            //printf("1:%d:%s:%d ", h, new_tmp, (strlen(sub_str[h]) + 1));
            new_tmp += (strlen(sub_str[h]) + 1);
            total_len += (strlen(sub_str[h]) + 1);
        }
        else
        {
            strcpy(new_tmp, new_str);
            //printf("2:%d:%s:%d ", new_str_index, new_tmp, (strlen(new_str) + 1));
            new_tmp += (strlen(new_str) + 1);
            total_len += (strlen(new_str) + 1);
        }
        h++;
    }
...
{
//h2g changed to h2g_nocheck
   ucontext_t *uc = (ucontext_t *)ptr;
   //printf("signal num:%d ", sig_num);
   //printf("signal raised at address:%x, %x ", si->si_addr, h2g_nocheck(si->si_addr));
   //printf("signal ip:%x\n", uc->uc_mcontext.gregs[REG_RIP]);
   //uc->uc_mcontext.gregs[REG_RIP] += 6;

   MISSING_PAGE page;
   page.addr = h2g_nocheck(si->si_addr);
   ...
   if((page.addr > 0x70000000 && page.addr < 0x80000000) || (page.addr>0x400000 && page.addr<0x1000000)){
   //if((page.addr < 0x80000000)){
   	//printf("write addr is:%x,prot:%d, err:%x\n", page.addr, page.prot, err); 
    ...
    write_addr(&page);
    re = read_addr(&page, &mmap_addr);

    int file_offset = (mmap_addr - hva_start) & 0xfffffffffffff000;
    if(page.prot == 0 || page.prot == 2)
    {
        uintptr_t res = target_mmap(h2g_nocheck(si->si_addr) & 0xfffff000, 1024*4, PROT_READ, MAP_SHARED | MAP_FIXED,  mem_file, file_offset);
    }
    else if(page.prot == 1)
    {
        uintptr_t res = target_mmap(h2g_nocheck(si->si_addr) & 0xfffff000, 1024*4, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED,  mem_file, file_offset);

    }
    else{
        printf("page prot error\n");
        sleep(10000);
    } 

    gettimeofday(&handle_addr_end, NULL);
    double handle_addr_time  =  (double)handle_addr_end.tv_sec - handle_addr_start.tv_sec + (handle_addr_end.tv_usec - handle_addr_start.tv_usec)/1000000.0;
    printf("map time:%f, addr:%x\n", handle_addr_time, page.addr);
    handle_addr_total += handle_addr_time;
    ...
    if(page.prot == 1)
    {
        //new
        gettimeofday(&store_page_start, NULL);
        store_page(h2g_nocheck(si->si_addr) & 0xfffff000, 1);
        addr_pair[addr_pair_index].virt_page = h2g_nocheck(si->si_addr) & 0xfffff000;
        addr_pair[addr_pair_index].phys_page = (mmap_addr - hva_start) & 0xfffffffffffff000;
        addr_pair_index++;
        add_physical_page((mmap_addr - hva_start) & 0xfffffffffffff000);
        add_store_write_page(h2g_nocheck(si->si_addr) & 0xfffff000);
    ...
    map_count++;
    //printf("%d mmap vaddr:%lx\n", map_count, h2g_nocheck(si->si_addr) & 0xfffff000);
 

/*
    if(((mmap_addr - hva_start) & 0xfffff000) == 0xeda6000)//0x4194b0
        {//grep '0000 2f00 7468 6f64 7363 772f'
           // grep 'fff5 1440 1821 0072 8324 8f82' cgibin
           // grep 'a36c 2484 f809 0320'cgibin 0x40959c
            char buf[0x1000];
            int tmp_len = 0x400; // origin buf len we want to read
            uintptr_t env_ptr =  (h2g_nocheck(si->si_addr) & 0xfffff000) + 0xcf3;
            ...
            int len = search_http_and_replace(buf, tmp_len, "HTTP_COOKIE=", new_buf, 0x1000);
            printf("#############new data is %s\n", buf);
            cpu_memory_rw_debug(NULL, env_ptr, buf, tmp_len, 1); //not len because len may less than tmp_len, connot cover the old content

        }
*/
    //printf("maping addr:%lx,%lx, phys:%lx\n", mmap_addr, si->si_addr,  (mmap_addr - hva_start) & 0xfffffffffffff000);


	if(is_fork) 
	{
		//afl_request_map(h2g_nocheck(si->si_addr) & 0xfffff000, (mmap_addr - hva_start) & 0xfffffffffffff000);
	}
	//printf("read mmap result:%x\n", res);
   } 
   else{
    is_fork = 0;
    ...
    */
	...
	//printf("other address error:%lx,%lx,%lx\n", si->si_addr, h2g_nocheck(si->si_addr),getpid());
/*
	int j, nptrs;
	#define SIZE 100
	void *buffer[100];
...
//ADDR STORE AND RESTORE
/*
typedef struct STORE_ADDR{
	uintptr_t addr; //host virtual addr
	int guest_addr;
	int data;
...
void store_addr(int guest_addr)
{

	int data = 0;
	cpu_memory_rw_debug(NULL, guest_addr, &data,  4, 0);
	uintptr_t addr = g2h(guest_addr);
	...
	if(tmp_p == NULL)
	{
		...
		st->addr = addr;
		st->data = data;
		st->guest_addr = guest_addr;
		if(guest_addr < 0x70000000) printf("**********store:%lx, %lx,%lx\n", guest_addr, addr, data);
		st->next = NULL;  
		data_pt[index] = st;
		return;
	}
...
void restore_addr()
{
	...
	for(int i=0; i<0x1000; i++)
	{
		...
		while(tmp_p){
			uintptr_t addr = tmp_p-> addr;
			unsigned long data = tmp_p-> data;
			int guest_addr = tmp_p -> guest_addr;
			//printf("**********restore addr: %lx, %lx data: %lx\n", guest_addr, addr, data);
			cpu_memory_rw_debug(NULL, guest_addr, &data,  4, 1); //data copy, cannot ensure env is in http
			STORE_ADDR * last = tmp_p;
			tmp_p = tmp_p->next;
			free(last);
		}
	}
...
/*
typedef struct STORE_PAGE_INDEX{
	uintptr_t addr;
	struct STORE_PAGE_INDEX * next;
} STORE_PAGE_INDEX;

STORE_PAGE_INDEX *pt_index[0x1000];

void store_index(uintptr_t guest_addr)
{
	int page_exist = 0;
	int index = (guest_addr >> 12) & 0xfff;
	STORE_PAGE_INDEX * tmp_p = pt_index[index];
	STORE_PAGE_INDEX * last_p;
	if(tmp_p == NULL)
	{
		STORE_PAGE_INDEX * page_index = malloc(sizeof(STORE_PAGE_INDEX));
		page_index->addr = guest_addr;
		page_index->next = NULL;  
		pt_index[index] = page_index;
		afl_request_storeaddr(guest_addr);
		return;
	}
	while(tmp_p!= NULL)
	{
		last_p = tmp_p;
		if(guest_addr == tmp_p->addr){
			page_exist = 1;			
			return;
		}
		tmp_p = tmp_p->next;
...
char mmaped_vaddr_bitmap[65536];
char stored_vaddr_page[65536];
char mmaped_paddr_bitmap[131071];
...
int find_virt_addr(int paddr, int * virt_addr)
{   
    for(int i=0; i<addr_pair_index; i++)
    {
        if(paddr == addr_pair[i].phys_page)
        {
            *virt_addr = addr_pair[i].virt_page;
            return i;
        }
    }
    return -1;
...
void flush_add_pair(int i)
{
    addr_pair[i].virt_page = 0;
    addr_pair[i].phys_page = 0;
}
...
/*
int add_write_mapped_vaddr(int vaddr)
{
    int value = vaddr >> 12;
    int index = value >> 3;
    int position = value & 0x07;
    mmaped_vaddr_bitmap[index] |=  1 << position;
}

int delete_write_mapped_vaddr(int vaddr)
{
    int value = vaddr >> 12;
    int index = value >> 3;
    int position = value & 0x07;
...
int if_vaddr_write_mapped(int vaddr) //vaddr <= 0x7ffff000
{   
    int value = vaddr >> 12;
    int index = value >> 3;
    int position = value & 0x07;
    return (mmaped_vaddr_bitmap[index] & (1 << position)) !=0; 
...
int add_mapped_paddr(int paddr)
{
    int value = paddr >> 12;
    int index = value >> 3;
    int position = value & 0x07;
    mmaped_paddr_bitmap[index] |=  1 << position;
}

int delete_mapped_paddr(int paddr)
{
    int value = paddr >> 12;
    int index = value >> 3;
    int position = value & 0x07;
...
int if_paddr_mapped(int paddr) //vaddr <= 0x7ffff000
{   
    int value = paddr >> 12;
    int index = value >> 3;
    int position = value & 0x07;
    return (mmaped_paddr_bitmap[index] & (1 << position)) !=0; 
...
int open_read_pipe()
{
    int res = 0;
    if(pipe_read_fd == -1){
        const char *fifo_name_full = "./pipe/full_cpu_state";  
...
{  
    ...
    if(pipe_write_fd == -1){
        ...
        pipe_write_fd = open(fifo_name_user, open_mode_user);  
        printf("write state open pipe:%x\n", pipe_write_fd);
    }
    */
    int type = 1;  
    if(pipe_write_fd != -1)  
...
int read_addr(MISSING_PAGE *page, uintptr_t *addr)  
{      
    int res = 0;  
/*    
    if(pipe_read_fd == -1){
...
*/   
    ...
    {    
        *addr = 0;
        ...
        //printf("read addr ok:%lx\n", page->addr);
/* 
        while(*addr == 0) //check
        {
            //printf("addr is to little\n");
            //sleep(100);
            res = read(pipe_read_fd, addr, sizeof(uintptr_t));  
        }
       
        if(*addr < 0xffffffff) // check
        {
            printf("addr is not right\n");
            stopWork_normal();
        }
        //zyw if addr is to little, read addr again
        while((page->addr & 0xfff) != ((*addr) & 0xfff))
        {
            printf("addr translation not correct:%lx,%lx,%lx\n", (page->addr & 0xfff), (*addr) & 0xfff), addr;
            read(pipe_read_fd, addr, sizeof(uintptr_t));  
        }
*/
    }  
    else {
...
int store_mapping(int gva, int hva)
{
	gva_arr[map_index] = gva;
	hva_arr[map_index] = hva;
	map_index++;
}

	
/*
int read_store_mapping()  
{  
    const char *fifo_name_full = "/pipe/full_cpu_state";  
    const char *fifo_name_user = "/pipe/user_cpu_state";  
    int pipe_fd = -1;  
    ...
    CPUArchState env_read;
    pipe_fd = open(fifo_name_full, open_mode_user);    
    if(pipe_fd != -1)  
    {   
	int gva, hva;
	read(pipe_fd, &gva, sizeof(int));  
	res = read(pipe_fd, &hva, sizeof(int)); 
	printf("read store mapping\n");
	remapping(gva, hva);
	if(res == -1)  
	{  
		fprintf(stderr, "Write error on pipe\n");  
