                               ...
                               bool fixed)
{
    ...
    while (1) {
        ...
        if(errno != 0)
            perror("mmap: ");
        printf("real_start:%lx, %lx, %lx\n", current_start, real_start, host_size);
        if (real_start == (unsigned long)-1) {
            return (unsigned long)-1;
        }

        /* Ensure the address is properly aligned.  */
        printf("real_start:%lx\n", real_start);
        ...
        if (!host_start || real_start == current_start) {
            printf("real_start:%lx,%lx,%lx\n", guest_start, real_start, real_start -guest_start, real_size);
...
//zyw
    ...
    printf("elfload.c we manually modify afl_entry_point:%x\n", afl_entry_point);
...
//zyw
	       printf("image name:%s, vaddr:%x, %x, %x\n", image_name, vaddr_ps, eppnt->p_filesz + vaddr_po, eppnt->p_offset - vaddr_po);
/*
    	    if(strcmp(image_name, "/lib/ld-uClibc.so.0") == 0){
    		  vaddr_ps = 0;
    		//vaddr_ps+=0x7717400;
    	    }
*/
