...
while ($j>0)
{
	...
	$lanip	= XNODE_get_var($ifname.".IPADDR");
	
	$phy = query($ifpath."/phyinf");
	$phyp = XNODE_getpathbytarget("/runtime", "phyinf", "uid", $phy, 0);
	$devname = query($phyp."/name");
	...
	while ($i < $cnt)
	{
		...
		if ($url!="")
		{
			...
			if ($poli == "ACCEPT")
			{
				...
				if($lanip!="")		
					fwrite("a", $START, "iptables -A ".$CHAIN." ".$timecmd." -m string --url ".$url." ".$inputcmd." -p tcp -j HIJACK --to-url ".$lanip."/web_filter_info.php\n");			
				else
					fwrite("a", $START, "iptables -A ".$CHAIN." ".$timecmd." -m string --url ".$url." ".$inputcmd." DROP\n");			
			}
			else if ($poli == "DROP")
			{
				/*add  accept rule*/
				fwrite("a", $START, "iptables -A ".$CHAIN." ".$timecmd." -m string --url ".$url." ".$inputcmd." -j RETURN \n");
			}
			XNODE_set_var($CHAIN.".USED", "1");
		}
	}
	if ($poli == "DROP")
	{
		...
		if($lanip!="")		
			fwrite("a", $START, "iptables -A ".$CHAIN." -m string --http_req ".$inputcmd." -p tcp -j HIJACK --to-url ".$lanip."/web_filter_info.php \n");
		else
			fwrite("a", $START, "iptables -A ".$CHAIN." -m string --http_req -j DROP \n");
	}
	$j++;
}
