...
function check_dhcp_config($path, $inetp)
{
	...
	if ($start_ip > $end_ip)
	{
		set_result("FAILED",$path."/start",i18n("The start host id of lease pool should be small than the end host id."));
		return;
	}
	...
	if ($start_ip == 0)
	{
		set_result("FAILED",$path."/start",i18n("The input range of host id is out of the boundary."));
		return;
	}
	if ($end_ip >= ipv4maxhost($mask))
	{
		set_result("FAILED",$path."/end",i18n("The input range of host id is out of the boundary."));
		return;
	}
	...
	{
		set_result("FAILED",$path."/leasetime",i18n("The input lease time could not be small than 1 minute ."));
		return;
	}
