...
function __is_only_letter_number_chars(str)
{
	for (var i=0;i < str.length;i++)
	{
	    if (str.charAt(i) >= '0' && str.charAt(i) <= '9')  continue;	/* 0~9 */
	    if (str.charAt(i) >= 'A' && str.charAt(i) <= 'Z')  continue;	/* A~Z */
	    if (str.charAt(i) >= 'a' && str.charAt(i) <= 'z')  continue;	/* a~z */
	    return false;
    }
	return true;
...
function is_blank_in_first_end(s)
{
		c=s.charAt(0);
		if(c==' ')
			return true;
...
function strchk_server_name(str)
{
	if (__is_str_in_allow_chars(str, 1, "~`!#$%^&()_-{}.")) return true;
	return false;
}
...
function strchk_radiusname(str)
{
...
function print_mac(n)
{
	...
	for(i=1;i<7;i++)
	{
		...
		str+="<input class=text type=text id="+n+i+" name="+n+i+" size=2 maxlength=2 value='' onkeyup=\"move_mac("+i+","+n+i+","+n+j+")\">";
		if(i!=6)        str+=" : ";
...
function AdjustHeight()
{
	...
	*/
	...
	//alert("tmp="+tmp);
	if(tmp > h) h = tmp;
	//parent.ifrMain.style.display = "block";
	parent.ifrMain.height = h;

	//if(parent.ifrMain.height != 530)
	//	parent.ifrMain.height = 530;
}
// *********************************** commjs end *****************************************************
<?
