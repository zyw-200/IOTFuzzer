<? /* vi: set sw=4 ts=4: */
echo "echo UP 5G interface ! > /dev/console\n";
...
$txpower    = query("/wlan/inf:2/txpower");
$wlxmlpatch_ap_pid = "/var/run/wlxmlpatch_a.pid";
...
$EmRadiusEapUser    =   "/wlan/inf:2/wpa/embradius/eap_user/index";
...
echo "ifconfig ".$wlanif_a." up\n";
echo "brctl addif br0 ".$wlanif_a."\n";
...
if ($multi_total_state == 1)
{
	...
	{      
		...
		{ 	            
			echo "ifconfig ath".$index1." up\n";
			echo "brctl addif br0 ath".$index1."\n";	
...
if ($multi_total_state == 1)
{
	...
	{      
		...
		if ($multi_ind_state==1)  
		{ 	                   
			echo "\necho UP MSSID-demon ath".$index1."... > /dev/console\n";
			...
			$wlxmlpatch_pid	= "/var/run/wlxmlpatch".$index1.".pid";
			echo "wlxmlpatch -S ath".$index1." /runtime/stats/wlan/inf:2/mssid:".$index." RADIO_SSID1_ON RADIO_SSID1_BLINK MADWIFI > /dev/console &\n";
			...
			{            
				...
				{
					// Passphrase //
				if(query("/wlan/inf:2/multi/index:".$index."/autorekey/enable")==1 && query("/runtime".$multi_ssid_path."/index:".$index."/passphrase")!="")//add for autorekey by log luo
                                {
					...
					"wpa_key_mgmt=WPA-PSK\n");
                                ...
                                if(query("/wlan/inf:2/multi/index:".$index."/autorekey/enable")==1)    {$wpapskformat = 1;}
...
//			            }
			...
			{                    
				...
				{            
					...
					*/
					...
					echo "iwconfig ath".$index1." key ".$iw_keystring."\n"; 
				}	
			}
			/*security ---*/
			...
			if ($igmpsnoop == 1){
				...
				echo "echo setwl ath".$index1." > /proc/net/br_igmp_ap_br0\n";
				echo "ap_igmp &> /dev/console\n";				
				echo "echo $! > /var/run/ap_igmp_".$index1.".pid\n";
			}
...
if ($wlan_ap_operate_mode==3 || $wlan_ap_operate_mode==4)
{
	...
	{   
		...
		{   
			...
			echo "wlxmlpatch -S ath".$index." /runtime/stats/wlan/inf:2/wds:".$index_mac." RADIO_SSID1_ON RADIO_SSID1_BLINK MADWIFI > /dev/console &\n";
			echo "echo $! > ".$wlxmlpatch_pid."\n";
			/* Jack add 13/02/08 --- wlxmlpatch_v2*/
			...
			{           
				...
				echo "HorS=\`rstrcmp -s ".$wlanmac_a." -S ".$wds_mac."`\n";
  ...
  //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~APMODE:demon  up~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
	echo "wlxmlpatch -S ".$wlanif_a." /runtime/stats/wlan/inf:2 RADIO_SSID1_ON RADIO_SSID1_BLINK MADWIFI > /dev/console &\n";
	...
	if ($wlan_ap_operate_mode != 1) // if not APC mode
	{
		$auth_mode = query("/wlan/inf:2/authentication");
		if ($auth_mode>1){  
			echo $IWCONF." essid \"".get("s",$WLAN_a."/ssid")."\"\n";
			require($template_root."/__auth_hostapd_a.php"); 
		}
		else {
			/*keep ori space */
		}
	...
	echo "sleep 5\n";
/*	echo "iwlist ".$wlanif_a." compare\n";	*/
	if($txpower == 2){
		echo "iwpriv wifi1 tpscale 1\n";
	}
	else if($txpower == 3){
		echo "iwpriv wifi1 tpscale 2\n";
	}
	else if($txpower == 4){
		echo "iwpriv wifi1 tpscale 3\n";
	}
	else {
		echo "iwpriv wifi1 tpscale 0\n";
	}
	...
	if ($igmpsnoop == 1 && $wlan_ap_operate_mode != 1){
		...
		$Brctl_igmp_a = 1; 	
		echo "echo enable > /proc/net/br_igmp_ap_br0\n";
		echo "echo setwl ath16 > /proc/net/br_igmp_ap_br0\n";
		echo "ap_igmp &> /dev/console\n";		
