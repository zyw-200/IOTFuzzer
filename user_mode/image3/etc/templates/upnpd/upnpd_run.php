...
if ($generate_start==1)
{
	if (query("/upnp/enable")==1)
	{
		...
		echo "xmldbc -A ".$DDIR."/WLANAccessPointDevice.xml > ".$VDIR."/WLANAccessPointDevice.xml\n";
		echo "xmldbc -A ".$DDIR."/WLANConfiguration1.xml > ".$VDIR."/WLANConfiguration1.xml\n";

		$Nfile = "/var/run/upnp_alive.sh";
		fwrite ($Nfile, "#!/bin/sh\n");
		fwrite2($Nfile, "APMODE=`xmldbc -g /wireless/ap_mode`\n");
		fwrite2($Nfile, "if [ \"$APMODE\" = \"0\" ]; then\n");
