...
/*
// do not write if the vaddr exists in mapping table
int write_mapping_ext(int gva, int hva)
{
	int gva_h = gva & 0xfffff000;
	int hva_h = hva & 0xfffff000;
	if(search_map_table(gva_h) == -1){
		store_mapping(gva_h, hva_h);
		write_mapping(gva, hva);
	}
}
//
*/
...
/*
	...
	if(pgd == httpd_pgd && vaddr < 0x80000000 && addend!=0) { 
		write_mapping_ext(vaddr, addend);
...
//zyw
    //if(afl_user_fork) printf("cputlb addend is %lx, vaddr:%x, address:%x\n", tn.addend, vaddr, address);
    if (prot & PAGE_READ) {
...
//
    ...
    int start_addr_code = env->tlb_table[mmu_idx][index].addr_code;
    ...
    int orig_addr_code = env->tlb_table[mmu_idx][index].addr_code;
    ...
    int next_addr_code = env->tlb_table[mmu_idx][index].addr_code;

    mr = iotlb_to_region(cpu, pd, iotlbentry->attrs);

    CPUArchState * new_env = env;
    int final_addr_code = env->tlb_table[mmu_idx][index].addr_code;
...
*/
//
    if((uintptr_t)p < (uintptr_t)0x100000000)
    {   
        printf("new_env:%lx, orig %x, %x, %x,%x env:%lx, pc:%lx, addr:%lx, %lx,mmu_idx:%d index:%x, addr_code:%x, addr_read:%x\n",new_env, start_addr_code, orig_addr_code, next_addr_code, final_addr_code, env, env->active_tc.PC, addr & (TARGET_PAGE_MASK | TLB_INVALID_MASK), env->tlb_table[mmu_idx][index].addend, mmu_idx, index, env->tlb_table[mmu_idx][index].addr_code, env->tlb_table[mmu_idx][index].addr_read);
        for(int j =0; j <= 255; j++)
        {//CPUTLBEntry
            uintptr_t t = env->tlb_table[0][j].addend;
            target_ulong read = env->tlb_table[0][j].addr_read;
            target_ulong write = env->tlb_table[0][j].addr_write;
            target_ulong code = env->tlb_table[0][j].addr_code;
                
            if(read!=-1)
            {
                printf("j:%d,%lx,%lx,%lx,%lx\n", j,read,write,code,t);
            }
