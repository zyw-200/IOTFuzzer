...
//zyw
	//if(user_stack_count) printf("tlb: %x,%x,%x,%x\n",addr, env_ptr->tlb_table[mmu_idx][index].addr_read, env_ptr->tlb_table[mmu_idx][index].addr_write, env_ptr->tlb_table[mmu_idx][index].addr_code);
	if (__builtin_expect(
...
DECAF_errno_t DECAF_memory_rw(CPUState* env, /*uint32_t*/target_ulong addr, void *buf, int len,
	...
	while (len > 0) {
		...
		//if(user_stack_count) printf("decaf_memory_rw:%x\n", phys_addr);
