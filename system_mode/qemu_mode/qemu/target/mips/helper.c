        ...
        if ((tlb->G == 1 || tlb->ASID == ASID) && VPN == tag && !tlb->EHINV) {
            ...
            if (rw != MMU_DATA_STORE || (n ? tlb->D1 : tlb->D0)) {
                ...
                *prot = PAGE_READ;
                ...
                //if(helper_flag == 1 && address < 0x70000000) printf("virtual:%x, physical:%x,n:%x\n",VPN,tlb->PFN[n], n);
                                    ...
                                    hwaddr physical_base)
{
    ...
    if (mapped < 0) {
        ...
        int ret = env->tlb->map_address(env, physical, prot, real_address, rw,
                                     access_type);
	//if(helper_flag == 1 && real_address < 0x70000000) printf("get_seg_physical_address is %x,%x\n", real_address, *physical);
	return ret;
    } else {
...
void r4k_invalidate_tlb (CPUMIPSState *env, int idx, int use_extra)
{
    ...
    //zyw
    //if(helper_flag == 1) printf("r4k_invalidate_tlb:%d\n",ASID);
