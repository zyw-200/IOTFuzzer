...
target_ulong mapped_page[0x1000];
int mapped_type[0x1000];
int mapped_mmuidx[0x1000];
int mapped_page_index = 0;

void add_mapped_page(target_ulong addr,int access_type, int mmu_idx){
  for(int i = 0; i < mapped_page_index; i++)
  {
    if((addr & 0xfffff000) == mapped_page[i])
    {
      return;
    }
  }
  mapped_page[mapped_page_index] = addr & 0xfffff000;
  mapped_type[mapped_page_index] = access_type;
  mapped_mmuidx[mapped_page_index] = mmu_idx;
  mapped_page_index++;
  //printf("tlb fill:%lx,%d,%d\n", addr & 0xfffff000, access_type, mmu_idx);
}

void print_mapped_page(FILE * fp)
{
  printf("mapped page ");
  for(int i=0; i<mapped_page_index; i++)
  {
    fprintf(fp, "%lx:", mapped_page[i]);
    fprintf(fp, "%d:", mapped_type[i]);
    fprintf(fp, "%d\n", mapped_mmuidx[i]);
  }
}
              ...
              int mmu_idx, uintptr_t retaddr)
{
    ...
    if(program_start)
    {
      ...
      if(pgd == httpd_pgd && addr < 0x80000000)
      {   
          if(mmu_idx == 2)
          {
             //add_mapped_page(addr, access_type, mmu_idx);
          } 
      }
    }
