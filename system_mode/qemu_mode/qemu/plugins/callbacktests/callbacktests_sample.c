...
// notice here
	if(pc== target_main_address[index]){
		main_start = 1;
	}
	char modname[512];
...
/*
	if (0 == funcmap_get_name_c(pc, pgd, &modname, &functionname)) {
		...
		if(strcmp(functionname, "__libc_accept") == 0 ) //strcmp(functionname, "accept") == 0 || 
		{
			...
			DECAF_printf("cur_pro:%s, function:%s,%s,%x\n", cur_process,functionname, tmpBuf, pc);
...
/*
	if(pc == 0x411e90 && strcmp(cur_process, "httpd") == 0) // accept 0x400ab0
	{
		accept_block = 1;
		DECAF_printf("cur_pro:%s, function:accept, pc:%x\n", cur_process, pc);	
		return;
	}
	if(pc == 0x409f48 && strcmp(cur_process, "httpd") == 0)//spawn //execv 0x400da0
	{ 
		target_ulong a0 = cpu->active_tc.gpr[4];//fd
		char tmpBuf[50];
		memset(tmpBuf, 0, 50);
		DECAF_read_mem(param->bb.env, a0, 50, tmpBuf);
		DECAF_printf("%s, spawn,%s,%x\n", cur_process, tmpBuf, pc);
		return;
			
	}
	if(pc == 0x407c18 && strcmp(cur_process, "httpd") == 0)//poll 
	{	
		DECAF_printf("cur_pro:%s, function:poll before,pc:%x\n", cur_process, pc);
		return; 

	}
	if(pc == 0x411910 && strcmp(cur_process, "httpd") == 0)//poll 
	{	
		DECAF_printf("cur_pro:%s, function:poll,pc:%x\n", cur_process, pc);
		return; 

	}
	if(pc == 0x411bb0 && strcmp(cur_process, "httpd") == 0)// read 0x403cf0
	{
//after snapshot
		target_ulong a0 = cpu->active_tc.gpr[4];//fd
		target_ulong a2 = cpu->active_tc.gpr[6];//nbytes
...
//NEED CHANGE
	...
	if(pc >= 0x411910 && pc < 0x411f80) //MIPS.stubs for httpd;
		return;
...
*/
	...
	if(pc == 0x400a70 && strcmp(cur_process, "httpd_sample") == 0) // accept 
	{
		accept_block = 1; //0x4041bc
		DECAF_printf("cur_pro:%s, function:accept, pc:%x\n", cur_process, pc);
		return;
	}
	if(pc == 0x400d40 && strcmp(cur_process, "httpd_sample") == 0)//execv 
	{ 
		target_ulong a0 = cpu->active_tc.gpr[4];//fd
		char tmpBuf[50];
		memset(tmpBuf, 0, 50);
	...
	if(pc == 0x4036c0 && strcmp(cur_process, "httpd_sample") == 0)// read 
	{
//after snapshot
		target_ulong a0 = cpu->active_tc.gpr[4];//fd
		target_ulong a2 = cpu->active_tc.gpr[6];//nbytes
...
//NEED CHANGE
	if(pc == 0x403720 && strcmp(cur_process, "httpd_sample") == 0)// sleep
	{
...
//NEED CHANGE
	...
	if(accept_block == 1 && pc == 0x4002b8){ // after accept //&& 0x406a18 
		accept_block = 0;
	...
	else if(network_read_block == 1 && pc == 0x400308){//after read // &&0x4072f8, 
