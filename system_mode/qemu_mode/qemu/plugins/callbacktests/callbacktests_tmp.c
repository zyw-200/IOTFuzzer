...
//NEED CHANGE
	...
	if(pc == 0x400a60 && strcmp(cur_process, "httpd_sample") == 0) // accept 
	{
		accept_block = 1; //0x4041bc
		DECAF_printf("cur_pro:%s, function:accept, pc:%x\n", cur_process, pc);	
		return;
	}
	if(pc == 0x400d30 && strcmp(cur_process, "httpd_sample") == 0)//execv 
	{ 
		target_ulong a0 = cpu->active_tc.gpr[4];//fd
		char tmpBuf[50];
		memset(tmpBuf, 0, 50);
	...
	if(pc == 0x4036b0 && strcmp(cur_process, "httpd_sample") == 0)// read 
	{
//after snapshot
		target_ulong a0 = cpu->active_tc.gpr[4];//fd
		target_ulong a2 = cpu->active_tc.gpr[6];//nbytes
...
//NEED CHANGE
	if(pc == 0x403710 && strcmp(cur_process, "httpd_sample") == 0)// sleep
	{
