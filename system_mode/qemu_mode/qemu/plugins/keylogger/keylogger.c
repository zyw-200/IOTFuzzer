...
extern plugin_interface_t keylogger_interface;
static int taint_key_enabled=0;

/*DECAF_Handle*/ uintptr_t keystroke_cb_handle = DECAF_NULL_HANDLE;
/*DECAF_Handle*/ uintptr_t handle_write_taint_mem = DECAF_NULL_HANDLE;
/*DECAF_Handle*/ uintptr_t handle_read_taint_mem = DECAF_NULL_HANDLE;
/*DECAF_Handle*/ uintptr_t handle_block_end_cb = DECAF_NULL_HANDLE;
...
void check_call(DECAF_Callback_Params *param)
{
	...
	{
		DECAF_read_mem(env, env_ptr->regs[R_ESP], 4, &sys_call_ret_stack[stack_top]);
...
{
	uint32_t eip = DECAF_getPC(current_cpu);
	uint32_t cr3 = DECAF_getPGD(current_cpu);
	char name[128];
